...
- `daily-logs.md`（meta-root）→ ~~`DEFER: 与 plan3 iCloud symlink 物理迁入同步`~~ → **`DROP: 不迁`**（2026-05-23 user 决定：daily/experience 物理迁入作废，后续不从 meta 迁入；与 `docs-organization.md`「daily/experience 走 mono-native、不从 meta 迁入」一致。mono `docs/daily` + `docs/experience` 走 mono-native per docs-organization.md，无 iCloud symlink）
- `experience-docs.md`（meta-root）→ ~~`DEFER: 与 plan3 iCloud symlink 物理迁入同步`~~ → **`DROP: 不迁`**（同上，2026-05-23 作废；与 `docs-organization.md`「experience 不从 meta 迁入」一致）
