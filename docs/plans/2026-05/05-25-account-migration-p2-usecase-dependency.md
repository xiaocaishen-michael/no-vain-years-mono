...
> ✅ **本子 plan 已完结(2026-05-31)**:依赖/顺序/业务级调研全部产出并被 [子 plan 3](05-25-account-migration-p3-usecase-steps.md) 消费;mbw-account 迁移已收口(批 A-D ship、批 E realname 废弃),见 [master](05-25-account-migration-master.md)。下文留作迁移留痕。

## 1. Context

mbw-account 后端 16 use case 从旧 Java/Spring 迁移到 mono(NestJS + Prisma)。Plan 1 PoC(`UnifiedPhoneSmsAuth`)+ 批 A(`002-account-profile`)+ 批 B(`003-tokens`:RefreshToken + LogoutAllSessions,#196)+ 批 C(`004-account-deletion`:5 use case,#198)+ 批 D(`005-device-management`:ListDevices + RevokeDevice,#201,server-only — mobile 登录管理屏延后到 settings shell)已 ship。**原批 E(realname 实名认证,3 use case)已于 2026-05-31 废弃,不再迁** —— Plan 2 迁移收口于批 A-D(13/16 use case)。
...
W1.4 `db pull` 把旧 Java 业务表反推进 `apps/server/prisma/schema.prisma`:`account` / `credential` / `account_sms_code` / `refresh_token`(均 `@@schema("account")`) + `outbox_event`(`public`)。**剩余 use case 迁移 = 业务逻辑 + repository + controller + 测试,不是表 schema**。
...
**待建**(随对应批次):outbox **真消费方**(批 C)/ ip2region geo(批 D)。

### 4.2 实体读写矩阵(谁写 / 谁读 Prisma model)

| model | 写入方 use case | 读取方 use case |
|---|---|---|
| `account` | UnifiedPhoneSmsAuth(create+lastLogin)✅ · UpdateDisplayName✅ · DeleteAccount(→FROZEN)✅ · CancelDeletion(→ACTIVE)✅ · AnonymizeFrozenAccount(→ANONYMIZED)✅ | GetAccountProfile✅ · RefreshToken✅ · DeleteAccount✅ · CancelDeletion✅ |
...
7. **DeleteAccount(→FROZEN) → AnonymizeFrozenAccount**(freezeUntil 过期后 scheduler 转 ANONYMIZED;与 CancelDeletion **悲观锁互斥**)— ✅ 均已 ship(批 C)
...
顺序逻辑:批 A/B 立 account + token 基座 → 批 C 依赖 token 基座做删除/冻结 → 批 D 复用 refresh_token 读侧。**批 D 在批 B 完成后可与批 C 并行**(无共享可变状态)。**当前状态(2026-05-31):批 A/B/C/D 已 ship(D=#201 设备列表+撤销,server-only,mobile 登录管理屏延后到 settings shell),基座 + 删除/冻结 + 设备管理全落。原批 E(realname 实名认证)已于 2026-05-31 废弃,不再迁 —— Plan 2 迁移收口于批 A-D(13/16 use case)。**
...
| 6 | 每个 use case 的 server spec ↔ app spec 合并约束 | ⬜ **open**:每 feature `/speckit-specify` 前 user 给(硬 gate,详 master) |
...
- ☐ § 3 业务调研 7 维框架 + 9 use case 业务规则概览表齐全
- ☐ § 4 依赖图自洽(实体读写矩阵 + 有向链 + 批次顺序 + 并行性),无 TypeORM/Bucket4j 错栈词
- ☐ 迁移顺序结论:批 C 串行链、批 D 可与 C 并行 — 与依赖链一致
