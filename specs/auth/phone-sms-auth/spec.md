...
底部三方 OAuth 按钮（微信 / Google / Apple iOS-only） + 顶部 close `×` 按钮 + 底部"登录遇到问题"帮助链接占位 — **存在但 placeholder**，press 后弹"Coming in M1.3"toast，避免空 dead-end。Apple 在 Android 端**不渲染**（Platform.OS conditional render）。
...
3. **Given** login 页任意端，**When** press 顶部 close `×` 按钮，**Then** router.back（如有 history）/ 否则 noop
...
- **FR-S11（Outbox event）**：自动注册路径 publish `AccountCreatedEvent`（domain event，schema 见 plan.md）— 走 outbox pattern（domain event 持久化到 `outbox_event` 表，per 2026-05-17 W2.4 US2 decision；Spring Modulith 老 `event_publication` 表保留不动 → 后台 worker 异步分发），技术选型在 plan.md 决定（候选：自实现 outbox + Prisma + node-cron / BullMQ + Redis）
