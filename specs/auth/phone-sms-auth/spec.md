...
- **FR-S11（Outbox event）**：自动注册路径 publish `AccountCreatedEvent`（domain event，schema 见 plan.md）— 走 outbox pattern（domain event 持久化到 `event_publication` 表 → 后台 worker 异步分发），技术选型在 plan.md 决定（候选：自实现 outbox + Prisma + node-cron / BullMQ + Redis）
