...
- [X] T010 [US2] [Server] `RefreshTokenService.rotate(record, clientIp)` in `refresh-token.service.ts`：interactive `$transaction`（`isolationLevel:'Serializable'`）= 条件 revoke 旧（`updateMany where {id, revokedAt:null}`）→ `count===0` throw `UnauthorizedException('INVALID_CREDENTIALS')`（回滚）→ signAccess + generateRefresh + hash + create 新（继承 deviceId/deviceName/deviceType/loginMethod + 更 ipAddress + expiresAt=now+30d）；catch P2002 → 401；**外层 ≤3 次 P2034 retry**（镜像 `commit-phone-login.usecase.ts`，module-level `isWriteConflict`/`isPrismaUniqueViolation` helper）+ 构造器补 `JwtTokenService` DI（persist/find spec 同步加 jwt arg）+ 单测（Testcontainers 3 测：happy 撤旧插新+血缘继承+更 IP+30d+单 active / 单次使用 count===0→401 回滚 / 私网 IP→null）。**mock $transaction 略**（interactive tx mock 低价值高脆，Testcontainers 真测覆盖；并发/P2034 retry 见 T015）
