...
- [ ] T002 [P] [Server] `refresh-token-hasher.ts` in `apps/server/src/security/`：SHA-256 → 64 小写 hex（高熵 token 无 salt/HMAC，per ADR-0023 区分；禁 bcrypt）+ 单测（同输入稳定 / 大写 hex 拒 / 64 长度）
- [ ] T003 [P] [Server] `refresh-token.rules.ts` in `apps/server/src/security/`：纯函数 `isActive(record, now)`（`revokedAt==null && expiresAt>now`）/ `scrubPrivateIp(ip)`（私网/回环→null）/ `normalizeDeviceType(raw)`（→ phone/tablet/desktop/web/unknown）+ 常量 `REFRESH_TTL_DAYS=30` / `ACCESS_TTL_MIN=15`（与 `JwtTokenService` 对齐单一来源）+ 单测（表驱动覆盖私网/回环/公网 IP + 各 deviceType + active/expired/revoked）
