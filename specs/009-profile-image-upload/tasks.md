...
- [ ] T003 [P] [Server] `apps/server/src/config/oss.config.ts`（镜像 `sms.config.ts`）：Zod 校验 env `OSS_REGION` / `OSS_BUCKET` / `OSS_ACCESS_KEY_ID` / `OSS_ACCESS_KEY_SECRET`（凭证治理 per ADR-0037：最小权限 RAM 子账号、secret 不入码/日志/走部署 env）+ 导出 `ossPublicBaseUrl()` = `https://<bucket>.<region>.aliyuncs.com`。**同 task 加 `apps/server/.env.example` 的 `OSS_*` 契约块 + 本地 `apps/server/.env` 对齐**（否则 `check-env-sync` lefthook 拦 commit —— .env.example 键须在 .env 存在）。env 值/部署侧配置见 `docs/plans/2026-05/05-31-oss-provisioning-runbook.md`
