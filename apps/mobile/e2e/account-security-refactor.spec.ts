...
// US4: 昵称真实值 + 头像/性别/主页背景图 disabled 占位。
// US5: 安全卡 登录管理(005) / 注销账号(004) 导航不回归 + 安全小知识 disabled。
//
...
test('US1 — 三段卡片 row 集精确，实名认证/第三方账号绑定/二维码名片 不在 DOM', async ({ page }) => {
  ...
  // 安全卡 3 行
  for (const label of ['登录管理', '注销账号', '安全小知识']) {
    await expect(page.getByRole('button', { name: label, exact: true })).toBeVisible();
  }

  // 删除项 / 设计稿未引入项 0 出现（SC-001）
...
test('US5 — 登录管理(005) / 注销账号(004) 导航不回归，安全小知识 disabled', async ({ page }) => {
  await bootToAccountSecurity(page);
  // 登录管理屏挂载拉设备列表 → mock 空列表，避免未 mock 的 API 噪声
  await mockJson(page, DEVICES_LIST_URL, 200, { devices: [] }, 'GET');

  // 安全小知识 disabled
  await expect(page.getByRole('button', { name: '安全小知识', exact: true })).toBeDisabled();
